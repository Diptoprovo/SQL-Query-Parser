        ...
        while (i < input.length()) {
            ...
            if (Character.isLetter(c)) {
                ...
                switch (word) {
                    ...
                    case "ORDER":
                        if (i + 2 < input.length() && input.substring(i, i + 3).equalsIgnoreCase(" BY")) {
                            tokens.add(new Token(TokenType.ORDER, "ORDER"));
                            tokens.add(new Token(TokenType.BY, "BY"));
                            i += 3;
                            continue;
                        }
                        break;
                    ...
                    case "GROUP":
                        if (i + 2 < input.length() && input.substring(i, i + 3).equalsIgnoreCase(" BY")) {
                            tokens.add(new Token(TokenType.GROUP, "GROUP"));
                            tokens.add(new Token(TokenType.BY, "BY"));
                            i += 3;
                            continue;
                        }
